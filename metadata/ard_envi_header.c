...
NOTES:
  1. Only supports GEO, UTM, AEA, PS, and SIN projections.
  2. Only supports WGS84, NAD27, NAD83 datums (GEO, UTM, AEA, PS).
...
)
{
    ...
    /* Verify the projection is GEO, UTM, AEA, PS, or SIN and datum is
       WGS-84 */
    if (hdr->proj_type != ARD_GCTP_GEO_PROJ &&
        hdr->proj_type != ARD_GCTP_UTM_PROJ &&
        hdr->proj_type != ARD_GCTP_ALBERS_PROJ &&
        hdr->proj_type != ARD_GCTP_PS_PROJ &&
        hdr->proj_type != ARD_GCTP_SIN_PROJ)
    {
        sprintf (errmsg, "Unsupported projection code (%d).  GEO projection "
            "code (%d) or UTM projection code (%d) or AEA projection code "
            "(%d) or PS projection code (%d) or SIN projection code (%d) "
            "expected.", hdr->proj_type, ARD_GCTP_GEO_PROJ, ARD_GCTP_UTM_PROJ,
            ARD_GCTP_ALBERS_PROJ, ARD_GCTP_PS_PROJ, ARD_GCTP_SIN_PROJ);
        ard_error_handler (true, FUNC_NAME, errmsg);
        return (ERROR);
    }

    if (hdr->datum_type != ARD_WGS84 &&
        hdr->datum_type != ARD_NAD27 &&
        hdr->datum_type != ARD_NAD83 &&
        hdr->datum_type != ARD_NODATUM)
    {
        sprintf (errmsg, "Unsupported datum code (%d). WGS84 datum code (%d) "
            "or NAD27 datum code (%d) or NAD83 datum code (%d) or NODATUM "
            "datum code (%d) expected.", hdr->datum_type, ARD_WGS84,
            ARD_NAD27, ARD_NAD83, ARD_NODATUM);
        ard_error_handler (true, FUNC_NAME, errmsg);
        return (ERROR);
    }
    ...
    {
        case ARD_WGS84:  /* WGS84 sphere */
            strcpy (geogcs_str, "GCS_WGS_1984");
            strcpy (datum_str, "D_WGS_1984");
            strcpy (spheroid_str, "WGS_1984");
            strcpy (proj_datum_str, "WGS-84");
            semi_major_axis = ARD_GCTP_WGS84_SEMI_MAJOR;
            semi_minor_axis = ARD_GCTP_WGS84_SEMI_MINOR;
            inv_flattening = ARD_GCTP_WGS84_INV_FLATTENING;
            break;

        case ARD_NAD27:  /* Clarke 1866 sphere */
            ...
            semi_major_axis = ARD_GCTP_CLARKE_1866_SEMI_MAJOR;
            semi_minor_axis = ARD_GCTP_CLARKE_1866_SEMI_MINOR;
            inv_flattening = ARD_GCTP_CLARKE_1866_INV_FLATTENING;
            break;

        case ARD_NAD83:  /* GRS 1980 sphere */
            ...
            semi_major_axis = ARD_GCTP_GRS80_SEMI_MAJOR;
            semi_minor_axis = ARD_GCTP_GRS80_SEMI_MINOR;
            inv_flattening = ARD_GCTP_GRS80_INV_FLATTENING;
            break;
    }
    ...
    if (hdr->data_ignore_value != ARD_INT_META_FILL)
    ...
    else if (hdr->proj_type == ARD_GCTP_UTM_PROJ)
    {
        if (hdr->utm_zone > 0)
            fprintf (hdr_fptr,
    ...
    else if (hdr->proj_type == ARD_GCTP_ALBERS_PROJ)
    {
        fprintf (hdr_fptr,
    ...
    else if (hdr->proj_type == ARD_GCTP_PS_PROJ)
    {
        fprintf (hdr_fptr,
    ...
    else if (hdr->proj_type == ARD_GCTP_SIN_PROJ)
    {
...
/******************************************************************************
MODULE:  ard_create_envi_struct

PURPOSE:  Creates the ENVI header structure from the ARD global and band
tile_metadata.
...
NOTES:
  1. Only supports GEO, UTM, AEA, PS, SIN projections.
...
******************************************************************************/
int ard_create_envi_struct
(
    Ard_band_meta_t *bmeta,   /* I: pointer tile_metadata band metadata for
                                    this band */
    Ard_global_tile_meta_t *gmeta, /* I: pointer to global tile_metadata */
    Envi_header_t *hdr         /* I/O: output ENVI header information */
)
{
    char FUNC_NAME[] = "ard_create_envi_struct";   /* function name */
    char errmsg[STR_SIZE];   /* error message */
    int i;                   /* looping variable */
    ...
    {
        case ARD_INT8: hdr->data_type=1; break;
        case ARD_UINT8: hdr->data_type=1; break;
        case ARD_INT16: hdr->data_type=2; break;
        case ARD_UINT16: hdr->data_type=12; break;
        case ARD_INT32: hdr->data_type=3; break;
        case ARD_UINT32: hdr->data_type=13; break;
        case ARD_FLOAT32: hdr->data_type=4; break;
        case ARD_FLOAT64: hdr->data_type=5; break;
    }
    ...
    switch (gmeta->proj_info.proj_type)
    {
        ...
        default:
            sprintf (errmsg, "Unsupported projection type (%d).  GEO "
                "projection code (%d) or UTM projection code (%d) or AEA "
                "projection code (%d) or PS projection code (%d) or SIN "
                "projection code (%d) expected.", gmeta->proj_info.proj_type,
                ARD_GCTP_GEO_PROJ, ARD_GCTP_UTM_PROJ, ARD_GCTP_ALBERS_PROJ,
                ARD_GCTP_PS_PROJ, ARD_GCTP_SIN_PROJ);
            ard_error_handler (true, FUNC_NAME, errmsg);
            return (ERROR);
    }

    hdr->datum_type = gmeta->proj_info.datum_type;
    if (gmeta->proj_info.datum_type != ARD_WGS84 &&
        gmeta->proj_info.datum_type != ARD_NAD27 &&
        gmeta->proj_info.datum_type != ARD_NAD83 &&
        gmeta->proj_info.datum_type != ARD_NODATUM)
    {
        sprintf (errmsg, "Unsupported datum code (%d). WGS84 datum code (%d) "
            "or NAD27 datum code (%d) or NAD83 datum code (%d) or NODATUM "
            "datum code (%d) expected.", gmeta->proj_info.datum_type,
            ARD_WGS84, ARD_NAD27, ARD_NAD83, ARD_NODATUM);
        ard_error_handler (true, FUNC_NAME, errmsg);
        return (ERROR);
    }
